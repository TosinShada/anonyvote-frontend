...
import { genExternalNullifier, genSignalHash } from '@libsem/protocols'
...
export async function getIdentityCommitments() {
  const idCommitments = await semaphoreContract.getIdentityCommitments().catch((error) => {
    console.log(error)
  })
...
export async function broadcastSignal(signal, packedProof, root, nullifierHash, externalNullifier) {
  try {
...
export async function addPoll(poll) {
  try {
    const titleHash = genExternalNullifier(poll.title.replace(/\s+/g, ''))
    const tx = await semaphoreContract
    ...
    if (receipt.status === 1) {
      ...
      for (let index = 0; index < 3; index++) {
        ...
        request.options[index].hash = genSignalHash(signal).toString()
      }
      console.log(request)
      createPoll(request)
      return true
